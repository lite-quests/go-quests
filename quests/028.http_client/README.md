...
**The Golden Rule of Go HTTP Clients:** You should **never use the default HTTP client** (`http.Get`, `http.Post`) in production. The default client has *no timeout*, meaning if the server hangs, your application will hang forever waiting for a response! Instead, you must always instantiate a custom `http.Client`.
...
### Testing
```bash
go test -v ./quests/1005.http_client
```
